...
name: builtin-skills
description:
  Use when creating a new builtin skill for Hyper, editing an existing builtin
...
Hyper embeds skills directly into the binary via `internal/skills/builtin/`.
...
- `DiscoverBuiltin()` walks the embedded FS, parses each `SKILL.md`, and sets
  paths with the `hyper://skills/` prefix (e.g., `hyper://skills/jq/SKILL.md`).
- The View tool resolves `hyper://` paths from the embedded FS, not disk.
...
| -------------- | ----------------------- | ------------------------------------------ |
| `hyper-config` | `builtin/hyper-config/` | Hyper configuration help                   |
| `hyper-hooks`  | `builtin/hyper-hooks/`  | Authoring, configuring and debugging hooks |
