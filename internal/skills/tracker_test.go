...
func TestTracker_BuiltinSkillTracking(t *testing.T) {
	...
	// Simulate active skills including a builtin skill (hyper-config).
	activeSkills := []*Skill{
		{Name: "hyper-config", Description: "Hyper config", Builtin: true},
		{Name: "go-doc", Description: "Go docs", Builtin: false},
	}
	tracker := NewTracker(activeSkills)

	// Initially not loaded.
	require.False(t, tracker.IsLoaded("hyper-config"))
	require.False(t, tracker.IsLoaded("go-doc"))

	// Mark builtin skill as loaded (simulating read via hyper://...).
	tracker.MarkLoaded("hyper-config")
	require.True(t, tracker.IsLoaded("hyper-config"))
