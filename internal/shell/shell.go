...
// HyperEnvMarkers returns a fresh slice of the environment variables that
// Hyper unconditionally sets on every shell it spawns — both the interactive
...
func NewShell(opts *Options) *Shell {
	...
	// Allow tools to detect execution by Hyper.
	env = append(env, HyperEnvMarkers()...)
