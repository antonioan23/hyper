...
type RunOptions struct {
	...
	// the Hyper process cwd — hooks and the bash tool have different
...
// Hyper handler stack. Shared by the stateless [Run] entrypoint and the
...
//  1. builtins first (so Hyper's in-process jq wins over any PATH binary);
...
// builtinHandler returns middleware that dispatches recognized Hyper
