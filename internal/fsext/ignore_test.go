...
func TestHyperIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .hyperignore file that ignores .log files
	require.NoError(t, os.WriteFile(".hyperignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .hyperignore file
	hyperignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".hyperignore"), []byte(hyperignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .hyperignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .hyperignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .hyperignore in subdir that ignores normal_nested
	subHyperignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".hyperignore"), []byte(subHyperignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .hyperignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .hyperignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .hyperignore")
