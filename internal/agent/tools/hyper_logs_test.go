...
func createTestLogFile(t *testing.T, entries []map[string]any) string {
	t.Helper()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "hyper.log")
...
func TestNewHyperLogsTool(t *testing.T) {
	t.Parallel()
	tool := NewHyperLogsTool("/tmp/test.log")
	require.NotNil(t, tool)
	require.Equal(t, HyperLogsToolName, tool.Info().Name)
}

func TestHyperLogs_HappyPath(t *testing.T) {
	t.Parallel()
	...
	result := runHyperLogs(logFile, HyperLogsParams{Lines: 3})
...
func TestHyperLogs_DefaultLines(t *testing.T) {
	t.Parallel()
	...
	result := runHyperLogs(logFile, HyperLogsParams{Lines: 0})
...
func TestHyperLogs_MaxCap(t *testing.T) {
	t.Parallel()
	...
	result := runHyperLogs(logFile, HyperLogsParams{Lines: 200})

	lines := strings.Split(result, "\n")
	require.Len(t, lines, 100)
}

func TestHyperLogs_MissingFile(t *testing.T) {
	t.Parallel()
	result := runHyperLogs("/nonexistent/path/hyper.log", HyperLogsParams{Lines: 50})
	require.Contains(t, result, "No log file found")
}

func TestHyperLogs_EmptyFile(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "hyper.log")
	...
	result := runHyperLogs(logFile, HyperLogsParams{Lines: 50})
	require.Contains(t, result, "Log file is empty")
}

func TestHyperLogs_MalformedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "hyper.log")
	...
	result := runHyperLogs(logFile, HyperLogsParams{Lines: 10})
...
func TestHyperLogs_ExtraFieldsSorted(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runHyperLogs(logFile, HyperLogsParams{Lines: 1})
...
func TestHyperLogs_NonStringValues(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestHyperLogs_Redaction(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestHyperLogs_ReservedFields(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
		{
			"time":   time.Now().Format(time.RFC3339),
...
func TestHyperLogs_OversizedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "hyper.log")
...
func TestHyperLogs_PartialTrailingLine(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "hyper.log")
...
func TestHyperLogs_ValueQuoting(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestHyperLogs_ChronologicalOrder(t *testing.T) {
	t.Parallel()
...
func TestHyperLogs_TimeOnlyFormat(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestHyperLogs_LevelVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runHyperLogs(logFile, HyperLogsParams{Lines: 5})
...
func TestHyperLogs_SourceVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
