# Hyper Development Guide

## Project Overview

Hyper is a terminal-based AI coding assistant built in Go by
...
The module path is `github.com/charmbracelet/hyper`.
...
internal/
  ...
  config/
    ...
    load.go                        hyper.json loading and validation
  ...
  hooks/                           Hook engine: runs user shell commands on hook events
    ...
    input.go                       Stdin payload builder, env vars, stdout parsing (Hyper + Claude Code compat)
...
- **Context files**: Hyper reads AGENTS.md, HYPER.md, CLAUDE.md, GEMINI.md
...
- **Pub/sub**: `internal/pubsub` for decoupled communication between agent,
  UI, and services.
- **Hooks**: User-defined shell commands in `hyper.json` that fire before
