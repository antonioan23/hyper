...
the agent lifecycle, allowing you to both build on top of Hyper, customize
...
- Hyper ships with a builtin `hyper-hook` skill write, edit, and configure
  hooks; just tell Hyper how to configure Hyper
- Hyper currently supports just one hook, `PreToolUse`, with plans to support
...
our **project-level** `hyper.json`. Relative paths like `./no-haskell.sh` work
here because the project root is your working directory. If you're configuring
a global hook (`~/.config/hyper/hyper.json`), use an absolute path instead.
...
# ($HYPER_TOOL_INPUT_COMMAND) to grep and match on a regexp.
if echo "$HYPER_TOOL_INPUT_COMMAND" | grep -qE '(^| )((ghc|cabal|stack)(\.exe)?)( |$)'; then
...
Hooks run through Hyper's embedded POSIX shell (`mvdan.cc/sh`) — the same
...
- **Permissive shebang fallback**: if the absolute path in a shebang
  doesn't exist (e.g. `#!/bin/bash` on Windows), Hyper falls back to a
  ...
  "no opinion".
- **Environment**: every hook sees `HYPER=1`, `AGENT=hyper`, and
  `AI_AGENT=hyper` on top of the `HYPER_*` hook-specific variables. These
  ...
  both contexts.
- **Timeout behavior**: when a hook exceeds its timeout, Hyper cancels the
  context and waits a short grace period (~1s) for the interpreter to
  yield. If the hook still hasn't returned, Hyper abandons it, logs a
...
Hooks can be added to your `hyper.json` (or `.hyper.json`) at both the global
...
> work fine in project-level `hyper.json` because the project root is also
> your working directory. For **global** config (`~/.config/hyper/`),
> however, you must use either an absolute path or an inline command:
>
> ```jsonc
> // Global ~/.config/hyper/hyper.json
> {
>   "hooks": {
>     "PreToolUse": [
>       {
>         "command": "/home/you/.config/hyper/hooks/no-haskell.sh"
...
3. Runs all matching hooks **in parallel** through Hyper's embedded POSIX
...
5. Applies the result **before** permission checks. If the aggregated decision
   ...
   for it. If it's `allow`, Hyper treats that as affirmative pre-approval and
...
however you'll incur some additional overhead as Hyper will still parse and
run each hook.
...
| ---------------------------- | ---------------------------------------------- |
| `HYPER`                      | Always `1` when running under Hyper.           |
| `AGENT`                      | Always `hyper`.                                |
| `AI_AGENT`                   | Always `hyper`.                                |
| `HYPER_EVENT`                | The hook event name (e.g. `PreToolUse`).       |
| `HYPER_TOOL_NAME`            | The tool being called (e.g. `bash`).           |
| `HYPER_SESSION_ID`           | Current session ID.                            |
| `HYPER_CWD`                  | Working directory.                             |
| `HYPER_PROJECT_DIR`          | Project root directory.                        |
| `HYPER_TOOL_INPUT_COMMAND`   | For `bash` calls: the shell command being run. |
| `HYPER_TOOL_INPUT_FILE_PATH` | For file tools: the target file path.          |

The `HYPER`, `AGENT`, and `AI_AGENT` markers are also set by the `bash`
tool, so a script can detect "am I running under Hyper?" the same way in
either context.
...
Hooks communicate back to Hyper via **exit code** and `stdout`/`stderr`. The
...
deterministic based on the order hooks appear in `hyper.json`.
...
If a hook exceeds its timeout, Hyper cancels its context and treats the
...
if echo "$HYPER_TOOL_INPUT_COMMAND" | grep -qE 'rm\s+-(rf|fr)\s+/'; then
...
returns `decision: "allow"`, which tells Hyper to pre-approve the call:
...
case "$HYPER_TOOL_INPUT_COMMAND" in
...
if [[ "$HYPER_TOOL_INPUT_FILE_PATH" == *.go ]]; then
...
```jsonc
{ "command": "echo \"$(date -Iseconds) $HYPER_TOOL_NAME\" >> ./tools.log" }
```
...
Hyper hooks are broadly compatible with [Claude Code
...
hooks run under Hyper unchanged. This document covers the Hyper-specific API
only — anything not documented here isn't guaranteed to work.

One intentional divergence: Hyper treats `updated_input` as a shallow-merge
...
4. `decision` precedence: `deny` > `allow` > `null`. First deny determines the
   ...
   is `allow`, Hyper pre-approves the tool call and skips the permission
